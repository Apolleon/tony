...
export default function Game() {
  const { gameLevel: simpleLevelPlan } = useContext(GameContext);
  ...
  useEffect(() => {
    ...
    const { pressed: arrows, handler } = trackKeys(arrowCodes);

    function runLevel(level, Display, andThen) {
      const container = gameContainerRef.current;
      container.innerHTML = ""; // Очищаем контейнер перед созданием новой игры
      const display = new Display(container, level);

      const stopAnimation = runAnimation(function (step) {
    ...
    function runGame(plans, Display) {
      ...
      function startLevel(n) {
        if (cleanup) cleanup();
        cleanup = runLevel(new Level(plans[n]), Display, function (status) {
    ...
    const cleanupGame = runGame([simpleLevelPlan], DOMDisplay);
  ...
  }, [simpleLevelPlan]); // Добавляем зависимость

  return <section ref={gameContainerRef}></section>;
}
